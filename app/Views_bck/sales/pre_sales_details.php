                  ...
                  <?php } else{ $i=0; foreach(array_reverse($lead_details) as $lkey) { 
                    ...
                    ?>
                      <tr>
                        <td><?php echo ++$i;?></td>
                        <td>
                          <button type="button" data-toggle="modal" data-id="<?php echo $lkey['lead_id'].'*'.$lkey['lead_client'].'*'.$lkey['lead_dept'].'*'.$lkey['lead_contact_person']; ?>" data-target="#leadDetails" style="background-color: transparent;border:none;">
